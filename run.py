...
from src.read_winddata import read_windadata
...
def winddata2littler(logger, input_file, output_folder):
    """
    This function is used to convert winddata into littler format
    :param logger: the logger to log the whole process
    :param input_file: the path of raw hdf5 format winddata file
    :param output_folder: the folder where to save littler format files
    :return: None
    """
    winddata, date = read_windadata(input_file)
    output_path = f'{output_folder}/{date}'
    if not os.path.exists(output_path):
        os.mkdir(output_path)
        logger.info(f"Makeing output folder for {input_file}")
    ...
    for i in range(len(winddata)):
        hr = header_record(lat=winddata['lat'][i],
                           lon=winddata['lon'][i],
                           date=date)
        hr.write(file)
        del hr
        ...
        dr = data_record(wind_speed=winddata['wind_speed'][i],
                         wind_dir=winddata['wind_dir'][i])
        dr.write(file)
        del dr
        file.write('\n')
...
def main():
    ...
    parser.add_argument("--input_format", type=str, default="hy2b")
    ...
    logger.info("Start searching "+input_format+" files")
    input_files = os.listdir(input_folder)
    if input_format == "hy2b":
        qscat_filenames = []
    ...
    elif input_format == "winddata":
        wind_filenames = []
        for input_file in input_files:
            if os.path.splitext(input_file)[1] == '.hdf':
                wind_filenames.append(f'{input_folder}/{input_file}')
        logger.info("Winddata file paths are ready!")

        for wind_filename in wind_filenames:
            winddata2littler(logger, wind_filename, output_folder)
    else:
        logger.info("Can't convert this format, please check the data or running commond!")
        return
